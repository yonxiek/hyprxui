...
hl.config({
    general = {
        ...
        border_size = 2,
        col = {
            active_border   = { colors = {"rgba(333333ee)", "rgba(000000ee)"}, angle = 45 },
    ...
    decoration = {
        rounding = 12,
        active_opacity   = 1.0, -- Приложения теперь по умолчанию НЕ прозрачные
...
-- Правила для слоев (панель, меню, уведомления)
hl.layer_rule({ name = "blur", match = { namespace = "waybar" } })
hl.layer_rule({ name = "blur", match = { namespace = "notifications" } })
hl.layer_rule({ name = "blur", match = { namespace = "fuzzel" } })
hl.layer_rule({ name = "ignorealpha 0.2", match = { namespace = "waybar" } })

-- Правило для Kitty (включаем блюр через имя правила)
hl.window_rule({
    name  = "blur", 
    match = { class = "kitty" },
})

-- Дополнительно: установим прозрачность для Kitty здесь, 
-- если она не сработала в самом kitty.conf
hl.window_rule({
    name  = "opacity 0.85 0.75",
    match = { class = "kitty" },
})
